...
%random motion of particles resulting from collisions. 
...
%Scale Normal Distribution by K
...
%Simulated D
simulatedD = mean( dSquaredDisplacement ) / ( 2 * d * tau )
...
squaredDisplacement = ( x .^ 2) + ( y .^ 2);
simulatedD = mean( dSquaredDisplacement ) / ( 2 * d * tau )
standardError = std( dSquaredDisplacement ) / ( 2 * d * tau * sqrt(N) )
...
%Displacement Squared (Bulk Flow)
clf; hold on;
plot(time, (0:1:(N-1)) * 2*k^2 , 'k', 'LineWidth', 3);
plot(time, squaredDisplacement);
hold off;
...
particle{i}.D = mean( particle{i}.drsquared ) / ( 2 * d * tau );
particle{i}.standardError = std( particle{i}.drsquared ) / ( 2 * d * tau * sqrt(N) );
end

%Visualise Multiple Particles 
clf; hold on;
for i = 1:particleCount
...
xlabel('X position (m)');
ylabel('Y position (m)');
title('Combined Particle Tracks'); hold off;
...
clear D e dx;
% extract the D value from each simulation and place them all into a single matrix called 'D'
for i = 1:particleCount
D(i) = particle{i}.D;
dx(i,:) = particle{i}.dx;
...
% plot estimated D
plot((averageD + uncertainty) * ones(1,particleCount), 'g-', 'linewidth', 1);
...
%5000 data sets simulation (50 fewer @>=N)
clf;
hold on;
for i= 1:100
for j = 1:50
...
%Plotting multiple
%create an array whose columns contain the dx for each particle
for i = 1:particleCount
